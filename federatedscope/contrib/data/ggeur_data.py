...
- Distributed mode: uses pre-generated shards from distributed_loader
"""
...
def load_ggeur_data(config, client_cfgs=None):
    ...
    Supports two modes:
    1. Distributed mode (federate.mode='distributed' and
       distributed_data.enabled=True):
       - Uses pre-generated shards from distributed_loader
       - Each client loads its shard from disk
    ...
    2. Standalone mode:
       ...
       - If LDS is enabled, uses Dirichlet for non-IID data split
    ...
    Returns:
        ...
        data_dict: {client_id: {'train': dataloader, 'val':
                                 dataloader, 'test': dataloader}}
    """
    # Check if distributed mode is enabled
    if (hasattr(config, 'federate') and config.federate.mode == 'distributed'
            and hasattr(config, 'distributed_data')
            and config.distributed_data.enabled):

        logger.info("GGEUR using distributed mode with pre-generated shards")
        from federatedscope.core.data.distributed_loader import (
            load_distributed_data)
        return load_distributed_data(config)

    # Standalone mode: original logic
    ...
    if data_type == 'pacs':
        ...
        logger.warning(f"Data type {data_type} not specifically supported for "
                       f"GGEUR_Clip, falling back to standard loading")
        return None
...
def _generate_dirichlet_matrix(num_domains, num_classes, alpha, seed=42):
    ...
    Returns:
        Matrix of shape (num_domains, num_classes) where each column
        sums to 1
    """
    np.random.seed(seed)
    # Generate dirichlet distribution for each class
    # dirichlet([alpha] * num_domains, num_classes) returns
    # (num_classes, num_domains). We transpose to get
    ...
    logger.info(
        f"  Min proportion: {matrix.min():.4f}, Max: {matrix.max():.4f}")
...
def _split_dataset_with_lds(dataset, dirichlet_proportions, seed=42):
    ...
    Each class's samples are split according to the Dirichlet
    proportions. This creates a highly non-IID distribution where
    some classes may have very few samples for a given client.

    Args:
        dataset: Dataset with .data (paths) and .targets (labels)
        dirichlet_proportions: Array of proportions for this client
                                (one per class)
        seed: Random seed
    ...
    for class_idx in range(num_classes):
        ...
        proportion = dirichlet_proportions[class_idx] if class_idx < len(
            dirichlet_proportions) else 0
    ...
    logger.info(
        f"  LDS allocation: {total_allocated}/{total_available} samples "
...
def _split_subset_for_clients(subset, num_clients, seed=123):
    ...
    This is similar to _split_dataset_for_clients but works on Subset
    objects, which is needed when we first apply LDS and then split
...
def _load_pacs_ggeur_data(config, client_cfgs=None):
    ...
    # Default splits
    splits = tuple(config.data.splits) if hasattr(
        config.data, 'splits') else (0.8, 0.1, 0.1)
    ...
    use_lds = getattr(config.ggeur, 'use_lds', False) if hasattr(
        config, 'ggeur') else False
    ...
    if configured_client_num % num_domains != 0:
        logger.warning(
            f"client_num ({configured_client_num}) is not divisible by "
    ...
    if use_lds:
        ...
        logger.info(f"GGEUR_Clip PACS: {total_clients} clients, "
                    f"{clients_per_domain} per domain")
    ...
    for domain_idx, domain in enumerate(domains):
        ...
        try:
            domain_data = load_pacs_domain_data(root=root,
                                                domain=domain,
                                                splits=splits,
                                                transform=transform,
                                                seed=config.seed)
...
def _load_officehome_ggeur_data(config, client_cfgs=None):
    ...
    # Default splits
    splits = tuple(config.data.splits) if hasattr(
        config.data, 'splits') else (0.7, 0.0, 0.3)
    ...
    if use_lds:
        ...
        logger.info(
            f"GGEUR_Clip Office-Home with LDS: alpha={lds_alpha}, "
        ...
        logger.info(f"GGEUR_Clip Office-Home: {total_clients} clients, "
                    f"{clients_per_domain} per domain")
        dirichlet_matrix = None
    ...
    for domain_idx, domain in enumerate(domains):
        ...
        try:
            domain_data = load_office_home_domain_data(root=root,
                                                       domain=domain,
                                                       splits=splits,
                                                       transform=transform,
                                                       seed=config.seed)
            ...
            if use_lds:
                # LDS mode: first apply Dirichlet distribution to get
                # domain's portion
